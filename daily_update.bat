...
if "%DOW%"=="0" (
    echo [%date% %time%] Sunday - skipping. >> update_log.txt
    exit /b 0
)
if "%DOW%"=="6" (
    echo [%date% %time%] Saturday - skipping. >> update_log.txt
    exit /b 0
)
...
if "%LASTRUN%"=="%TODAY%" (
    echo [%date% %time%] Already ran today. Skipping. >> update_log.txt
    exit /b 0
)

if "%LASTRUN%"=="" (
    echo [%date% %time%] No previous run recorded - running now. >> update_log.txt
    goto :dorun
)
...
if %DAYSSINCE% GEQ 2 (
    echo [%date% %time%] Missed a previous day's run (last run: %LASTRUN%^) - catching up now. >> update_log.txt
    goto :dorun
)

REM DAYSSINCE == 1: normal case, only run once past 6:30 PM (1110 minutes).
if %NOWMIN% LSS 1110 (
    echo [%date% %time%] Before 6:30 PM and yesterday's run is up to date - not running yet. >> update_log.txt
    exit /b 0
)
